...
class SalesForce {
       ...
       await getSalesforceCred(currentShop).then(async (salescred)=>{
        console.log("i am in generate token");
         requestData = {
          ...
          client_secret: `${salescred.client_secret}`,
          username: `${salescred.username}`,
          refresh_token: `${salescred.refresh_token}`,
          grant_type: `${salescred.grant_type}`,
        };
       
        this.tokenApiUrl = `${salescred.token_api_url}`,
        this.contactApiUrl = `${salescred.contact_api_url}`,
        this.opportunityApiUrl = `${salescred.opportunity_api_url}`,
        this.opportunityContactRole = `${salescred.opportunity_contact_role}`,
        this.opportunityLineItem = `${salescred.opportunity_line_item}`,
        this.queryApiUrl = `${salescred.query_api_url}`
      }).catch((e)=>{
        console.log(e);
      });
      var res;
  ...
  async createSaleRecurring(orderData,next) {
    ...
    if (orderData) {
      ...
      if (opportunityExist && opportunityExist.totalSize > 0) {
        // console.log(JSON.stringify(opportunityExist) + "PARENT OPT");
  ...
  async #isOpportunityExist(OpportunityName) {
    ...
    console.log(apiUrl+' CAUSE ERROR URL')
    var res;
    try {
